...
class PaymentTransaction(Base):
    ...
    payment_session_token: Mapped[str | None] = mapped_column(Text, nullable=True)
    payment_session_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    status: Mapped[str] = mapped_column(String(40), default="pending", index=True)
    provider_status: Mapped[str | None] = mapped_column(String(60), nullable=True, index=True)
    ...
    marketplace_fee: Mapped[float] = mapped_column(Numeric(10, 2), default=0)
