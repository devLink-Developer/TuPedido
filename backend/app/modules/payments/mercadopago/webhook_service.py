...
def _load_order(db: Session, reference: str | None) -> StoreOrder | None:
    if not reference:
        return None
    return db.scalar(select(StoreOrder).options(*_order_options(db)).where(StoreOrder.payment_reference == reference))
...
def apply_payment_status(order: StoreOrder, status_value: str) -> None:
    current = str(order.payment_status or "pending")
    if current in {"paid", "approved"} and status_value in {"pending", "processing"}:
        return
    if current == "cancelled" and status_value in {"pending", "processing"}:
        return
...
def _extract_event_type(query_params: Any, payload: dict[str, Any]) -> str | None:
    return query_params.get("type") or query_params.get("topic") or payload.get("type") or payload.get("topic")


async def process_mercadopago_webhook(request: Request, db: Session) -> dict[str, object]:
    ...
    if event_type not in {None, "", "payment", "payment.created", "payment.updated"}:
    ...
    event_id = str(payload.get("id") or payment_id)
    ...
    transaction = find_transaction_by_payment_id(db, payment_id)
    if transaction is None and reference:
        transaction = find_transaction_by_reference(db, str(reference))
    order = _load_order(db, reference)
    store = _load_store(db, transaction.store_id) if transaction is not None else None

    store_id_raw = query_params.get("store_id") or payload.get("store_id")
    if store is None and store_id_raw:
        store = _load_store(db, int(store_id_raw))
        if store is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Store not found")
    if store is None and order is not None:
    ...
    transaction = transaction or find_transaction_by_reference(db, effective_reference) or find_transaction_by_payment_id(db, payment_id)
    if transaction is None:
        transaction = create_payment_transaction(
            ...
            marketplace_fee=getattr(order, "service_fee", 0),
        )
