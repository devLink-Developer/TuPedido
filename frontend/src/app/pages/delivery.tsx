    ...
    <article className="mesh-surface space-y-4 rounded-[32px] border border-white/80 p-5 shadow-lift">
      ...
      <div className="flex flex-wrap gap-2">
        {order.delivery_status === "assignment_pending" ? (
          <button type="button" onClick={onAccept} className="rounded-full bg-brand-500 px-4 py-3 text-sm font-semibold text-white">
            Aceptar pedido
          </button>
        ) : null}
        {order.delivery_status === "assigned" || order.delivery_status === "heading_to_store" ? (
          <button type="button" onClick={onPickup} className="rounded-full bg-ink px-4 py-3 text-sm font-semibold text-white">
            Confirmar retiro
          </button>
        ) : null}
        {order.delivery_status === "picked_up" || order.delivery_status === "near_customer" ? (
          <div className="flex flex-1 flex-wrap items-center gap-2">
            <input
              ...
              className="min-w-[160px] flex-1 rounded-full border border-black/10 bg-white px-4 py-3 text-sm"
            />
            <button
              type="button"
              onClick={() => onDeliver(otpCode)}
              disabled={delivering}
              className="rounded-full bg-emerald-600 px-4 py-3 text-sm font-semibold text-white disabled:bg-zinc-300"
            >
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <form onSubmit={(event) => void handleAuthSubmit(event)} className="mesh-surface space-y-4 rounded-[30px] border border-white/80 p-5 shadow-lift">
            <div className="flex gap-2">
              {(["register", "login"] as const).map((mode) => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-semibold ${authMode === mode ? "bg-ink text-white" : "bg-zinc-100 text-zinc-600"}`}
                >
            ...
            <button type="submit" disabled={saving} className="rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-white">
          ...
          <div className="rounded-[30px] bg-[linear-gradient(180deg,#221816_0%,#171210_100%)] p-6 text-white shadow-lift">
            ...
            <div className="mt-4 grid gap-3 text-sm text-white/75">
              <div className="rounded-[22px] border border-white/10 bg-white/5 px-4 py-4">Tracking en tiempo real para cliente y comercio.</div>
              <div className="rounded-[22px] border border-white/10 bg-white/5 px-4 py-4">Despacho por cercanía y mapa operativo mobile-first.</div>
              <div className="rounded-[22px] border border-white/10 bg-white/5 px-4 py-4">Liquidación de entregas, cobros cash y notificaciones del pedido.</div>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={(event) => void handleApplySubmit(event)} className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="mesh-surface grid gap-4 rounded-[30px] border border-white/80 p-5 shadow-lift md:grid-cols-2">
          ...
          <select value={form.vehicle_type} onChange={(event) => setForm((current) => ({ ...current, vehicle_type: event.target.value as DeliveryFormState["vehicle_type"] }))} className="rounded-2xl border border-black/10 bg-zinc-50 px-4 py-3">
        ...
        <div className="rounded-[30px] bg-[linear-gradient(180deg,#221816_0%,#171210_100%)] p-6 text-white shadow-lift">
          ...
          <div className="mt-4 grid gap-3 text-sm text-white/75">
            <div className="rounded-[22px] border border-white/10 bg-white/5 px-4 py-4">Identidad, contacto y disponibilidad operativa.</div>
            <div className="rounded-[22px] border border-white/10 bg-white/5 px-4 py-4">Licencia, patente y seguro para moto o auto.</div>
            <div className="rounded-[22px] border border-white/10 bg-white/5 px-4 py-4">Acceso a tracking, notificaciones y flota solo cuando el alta esté aprobada.</div>
          </div>
          {error ? <p className="mt-4 rounded-[22px] bg-rose-500/15 px-4 py-3 text-sm text-rose-100">{error}</p> : null}
          <button type="submit" disabled={saving} className="mt-4 rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-white">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="space-y-3">
          {notifications.map((notification) => (
            <article key={notification.id} className="rounded-[26px] bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                {!notification.is_read ? <span className="rounded-full bg-brand-100 px-2 py-1 text-[10px] font-semibold text-brand-700">Nueva</span> : null}
              </div>
