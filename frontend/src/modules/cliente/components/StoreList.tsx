          ...
          <div className="space-y-4 p-5">
            ...
            <div className="flex flex-wrap gap-2 text-xs font-semibold">
              ...
              {store.primary_category ? (
                <span className="rounded-full bg-orange-100 px-3 py-1 text-orange-700">{store.primary_category}</span>
              ) : null}
            </div>
            <div className="flex flex-wrap gap-2 text-xs font-semibold text-zinc-500">
              <span className="rounded-full bg-zinc-100 px-3 py-1">
                {store.delivery_settings.delivery_enabled
                  ? `Envío ${formatCurrency(store.delivery_settings.delivery_fee)}`
                  : "Solo retiro"}
              </span>
              <span className="rounded-full bg-zinc-100 px-3 py-1">
