  ...
  return (
    <section className="app-panel p-5">
      <div className="flex flex-col gap-3 border-b border-[var(--color-border-default)] pb-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-zinc-400">Listado</p>
          <h2 className="mt-2 text-xl font-bold text-ink">Productos cargados</h2>
          <p className="mt-2 text-sm text-zinc-600">Revisá disponibilidad, stock y precio final antes de publicar cambios.</p>
        </div>
      ...
      <div className="mt-4 space-y-3">
        {products.map((product) => (
          <article key={product.id} className="rounded border border-black/5 bg-white p-4 shadow-sm">
            <div className="flex gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold text-zinc-500">
                  <span className="rounded bg-zinc-100 px-3 py-1">Stock: {product.stock_quantity ?? "Sin control"}</span>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={() => onEdit(product)} className="rounded bg-zinc-100 px-3 py-2 text-xs font-semibold text-zinc-700">
                Editar
              </button>
              <button type="button" onClick={() => void onDelete(product.id)} className="rounded bg-rose-500 px-3 py-2 text-xs font-semibold text-white">
                Eliminar
              </button>
            </div>
          </article>
        ))}
        {!products.length ? <div className="rounded bg-white p-5 text-sm text-zinc-500 shadow-sm">Todavía no cargaste productos.</div> : null}
      </div>
    </section>
  );
}
